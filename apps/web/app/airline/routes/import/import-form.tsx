...
import { bulkImportRoutes, type BulkImportResult } from '../actions';
...
const REQUIRED_HEADERS = ['flight_number', 'departure_icao', 'arrival_icao'] as const;

/**
 * Simpler CSV-parser. Handled:
 *  - BOM strip (Excel speichert UTF-8 mit BOM)
 *  - \r\n und \n line-endings
 *  - Leere zeilen ignorieren
 *  - Quoted-fields mit commas innerhalb (RFC-4180 minimal subset)
 *  - Doppelte quotes als escape ("" → ")
 *
 * Bewusst KEIN PapaParse als dependency — der parser hier ist ~30 zeilen
 * und covered die 99%-fälle. Wenn jemand ein CSV mit embedded-newlines
 * in quoted-fields hat, müssten wir auf eine library wechseln, aber
 * für route-data (alles short identifiers) ist das nicht relevant.
 */
...
type PreviewState =
  ...
  | { status: 'imported'; result: BulkImportResult };
...
export function ImportForm() {
  ...
  function handleFile(file: File) {
    ...
    reader.onload = (e) => {
      ...
      // Header normalisieren (lowercase, trim) für matching
  ...
  function handleConfirm() {
    ...
    startTransition(async () => {
      const result = await bulkImportRoutes(rows);
            ...
            <table className="min-w-full text-xs">
              <thead className="bg-gray-50 dark:bg-gray-800/50">
                <tr>
                  ...
                  <th className="text-left py-2 px-3 font-semibold">Flugnr.</th>
              ...
              <tbody>
                {result.rows.map((r) => (
                  ...
                  >
                    ...
                    <td className="py-2 px-3 font-mono">{r.flightNumber ?? '—'}</td>
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6 shadow-sm space-y-4">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex gap-2">
            ...
            >
              {isPending ? 'Importiere…' : `${state.rows.length} routes importieren`}
            </button>
          </div>
        </div>

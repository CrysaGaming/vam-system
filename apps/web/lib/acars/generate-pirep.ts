 ...
 *
 * Status = Draft (option #19, Edit-Before-Submit). The auto-PIREP
 * lands as Draft so the pilot gets a chance to review the splice-
 * generated remarks (sim-rate flags, replay-flags, "auto-filed"
 * preamble) before broadcasting to Discord and the admin-queue.
 * The pilot submits via /pireps/[id] when satisfied — that's the
 * Draft → Submitted transition that fires emitPirepSubmitted +
 * evaluatePromotion. Without this gate the Discord channel got a
 * notification for every flight the moment BLOCK_ON fired, which
 * was noisy when the heuristics produced a false-positive flag
 * the pilot wanted to remove first.
 *
 * Manual /pireps/new submissions skip Draft entirely: the pilot
 * already typed the remarks themselves, no review step is needed.
 */
...
export async function generatePirepFromSession(
  ...
  //
  // Suffix wording (option #19): "Draft auto-generated" rather than
  // "Auto-filed", because we now land in Draft and the pilot is
  // expected to review before submitting. The phrase is what the
  // pilot will read on /pireps/[id] — calling it "filed" while it's
  // still in Draft would be misleading. The pilot can edit this
  // text wholesale before submitting if they want.
  ...
  const replayFlagPrefix =
    replayFlags.length > 0
      ...
      : '';
  const remarks =
    `${acarsFlagPrefix}${replayFlagPrefix}Draft auto-generated by ACARS-client at block-on (callsign ${session.callsign}).`;

  // ─── INCIDENT lookup BEFORE the transaction (option #20) ───────────
  // Two callers consume this: (1) the structured Pirep.flags column
  // we write inside the tx (option #20), and (2) the Discord embed
  // enrichment fields we return after the tx (option #17). Doing the
  // lookup once here keeps it to one query and lets the tx-internal
  // pirep.create write the structured flags atomically rather than
  // needing a follow-up update.
  //
  // Read-only against already-committed AcarsEvent rows (the INCIDENT
  // was written from the heartbeat-route at M3.9 detection-time, not
  // from this helper). Pre-tx is fine — we don't risk reading our own
  // un-committed writes.
  ...
  // Build the structured flags object (option #20). Returns undefined
  // if no flags fired; we pass that straight to Prisma which writes
  // SQL NULL into the column. The thresholds (simRate > 1.01,
  // pauseSec > 60) are intentionally identical to the remarks-prefix
  // splicing above — a pilot reading the prefix and an admin querying
  // flags should agree on what counts as flagged.
  const pirepFlags = buildPirepFlags({
    simRate: session.simRate,
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const pirep = await tx.pirep.create({
      data: {
        ...
        // Option #19: Draft (not Submitted). Pilot reviews remarks +
        // optionally edits flightTimeMin/fuelUsedKg via the /pireps/[id]
        // edit-form, then explicitly submits via submitDraftPirep
        // (apps/web/app/pireps/actions.ts). Side-channels (Discord embed,
        // rank-promotion) only fire on that Draft → Submitted transition.
        status: 'Draft',
        ...
        landingRateFpm,
        remarks,
        // Option #20: structured anti-cheat flags. `pirepFlags` was
        // assembled before the tx (buildPirepFlags) and is undefined
        // when no flags fired — Prisma treats undefined as "don't write
        // this field" so the column stays SQL NULL for clean PIREPs.
        // Cast to InputJsonValue because Prisma's Json input type
        // doesn't auto-narrow our object literal.
        flags: pirepFlags as Prisma.InputJsonValue | undefined,
  ...
  // ─── Discord embed enrichment derivation (option #17) ──────────────
  // We already fetched the INCIDENT row before the transaction (so the
  // structured `flags` JSON for option #20 could include hardLanding
  // without a follow-up update). Re-use that same `incidentEvent` here
  // for the bot-payload — single query feeds both surfaces.
